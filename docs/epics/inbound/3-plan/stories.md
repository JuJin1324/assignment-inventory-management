...
- **S1** — 재고→상품 통합·식별자 모델(`productId` String → 과제 `id`)
- **S2** — 컨트롤러·서비스 입자 규약 (백로그 결정 메모 승급)
- **S3** — 미등록 상품 판별 키 (find-or-create가 무엇으로 "이미 있음"을 가르나)
...
- **재고→상품 통합** — 상품이 수량을 직접 드는 단일 애그리거트로 합친다(`structure`의 상품·재고 분리를 MVP에서 합침). ADR-004(도메인=JPA 하나로)·ADR-005(수량 컬럼)는 대상이 상품으로 바뀔 뿐 유지.
- **식별자 모델** — 기존 `productId`(String) 비즈니스 키를 유지할지, 과제 `id`(대리 키) 모델로 갈지. 입고의 미등록 판별 키(S3)와 맞물리므로 그 결과를 함께 본다.
...
- **미등록 상품 판별 키** — find-or-create가 무엇으로 "이미 있음"을 가르나(`id` / 비즈니스 키 / `name`). S1 식별자 모델과 맞물리므로 함께 본다.
