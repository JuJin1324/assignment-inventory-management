...
graph LR
    ...
    S5["Story 5<br/>출고 계층 구조 모델링 (비코딩)"]
    S1 --> S2 --> S3 --> S4 --> S5
```

도메인-퍼스트로 간다. 출고의 핵심 로직(재고 차감·음수 불가)을 DB·웹 없이 순수 도메인 + 단위 테스트로 먼저 못 박고(S1), 그 위에 RDBMS를 도입해 영속한다(S2). 그 위에 서비스 계층이 출고를 엮고(재고 조회 → 차감 → 저장, S3), REST API로 노출한다(S4). 레이어드(도메인 → 영속 → 서비스 → API)로 한 겹씩 쌓는다. 흐름이 다 선 뒤, 구현된 출고의 계층 구조를 모델링에 더해 에픽을 닫는다(S5). 동시성은 그 뒤 *동시 재고 변경 충돌* 에픽에서 RDBMS 위에 얹는다.
...
## Story 5 — 출고 계층 구조 모델링 (비코딩)

### 목적

S1~S4로 구현된 출고가 *어떤 계층으로 섰는지*를 모델링에 더한다. 처음 모델링은 추상 수준(시스템·저장소)에서 흐름만 봤지만, 이제 실제 계층 구조가 섰으니 이를 별도 구조 문서로 담는다. 시나리오(이해관계자 흐름)는 추상 수준을 유지하고, 계층 통신은 구조 문서가 맡는다. 코드 변경은 없다 — 문서만 더한다.

### 실행 완료 기준

- 출고 구현의 계층 구조(컨트롤러 → 서비스 → 도메인 → 리포지토리)가 별도 구조 문서로 모델링에 더해진다 — *다이어그램 → 설명 → 각 컴포넌트* 3단 형식
- 그 문서가 도메인 개념 구조(`structure.md`)와 분리된다 — `structure.md`는 *무엇이 있나*, 새 문서는 *출고가 어떤 계층으로 섰나*
- 시나리오는 이해관계자 흐름의 추상 수준을 유지한다 — 계층 통신을 시나리오에 욱여넣지 않는다(네 결과를 분기로 다 담으면 중첩이 깊어져 지저분)
- 시스템 전체 비전(입고·조회·이력 시나리오, Product·Movement 개념)은 그대로 둔다

### 분해

비코딩 Story라 plan-tasks를 건너뛰고 바로 execute-task로 실행한다. 산출물: [docs/modeling/structure-ship.md](../../../modeling/structure-ship.md).
