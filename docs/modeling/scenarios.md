...
sequenceDiagram
    ...
    alt 충돌 감지
        ...
        System->>Store: B — 최신 수량 재조회 · 차감 · 저장
        System-->>StaffB: 출고 완료 (남은 수량)
    end
```

### 흐름 설명

두 담당자가 같은 상품에 동시에 출고를 요청하면, 두 트랜잭션이 같은 수량을 읽고 각자 차감을 시도한다. 한 쪽이 먼저 저장하면 나머지는 충돌을 감지하고 최신 수량을 다시 읽어 재시도한다. 재시도 후 수량이 충분하면 출고가 완결되고, 부족하면 정상 거부로 닫힌다.

### 흐름 특유의 모양

- 두 트랜잭션이 같은 수량을 읽는 순간이 충돌의 씨앗이다 — *읽기·쓰기 사이의 간격*에서 lost update가 발생한다
- 시스템이 충돌을 먼저 탐지해야 한다 — 탐지 없이 덮어쓰면 한 트랜잭션의 변경이 사라진다
- 재시도 후 최신 수량 기준으로 다시 검증한다 — 충돌 이전 값으로 차감하지 않는다
